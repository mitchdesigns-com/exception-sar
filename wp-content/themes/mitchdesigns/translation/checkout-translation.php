            ...
            if(str_contains($err , 'minimum spend')){
                ...
                <script>
                    if($('body').hasClass('rtl')) {
                        $('.woocommerce-notices-wrapper').html('<ul class="woocommerce-error" role="alert"><li><?php echo " الحد الأدنى للإنفاق لهذه القسيمة هو ".  $parm->minimum_amount ."ر.س " ;?></li></ul>');
                    }else{
                        <?php
                        $err = "The minimum amount for this coupon is " . $parm->minimum_amount . " SAR" ;
                        ?>
  ...
  if(isset($_COOKIE['current_language'])){
	...
	if ($language == "en") {
	  //$coupon_html = str_replace('[حذف]', '[Remove]', $coupon_html);
		...
		$coupon_html = str_replace('SAR', 'ر.س', $coupon_html);
