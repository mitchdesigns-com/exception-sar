...
<div id="product_<?php echo $product_data['product_id']; ?>_block" class="product_widget">
	...
	?>
	<a class="product_widget_box <?php if ($Excluded) {
										echo 'excluded';
									} ?> "
		...
		<div class="img <?php echo ($product_data['product_flip_image']) ? 'has-flip' : '' ?>">
			...
			<!-- <?php //if(!empty($product_data['product_flip_image'])){ 
					?>
          <img  class="flip" src="<?php //echo $product_data['product_flip_image'];
									?>" alt="">
          <?php //}
			?> -->
					...
					<?php if ($Excluded == false) { ?>
						...
						<?php if ($product_data['product_type'] == 'simple') { ?>
							<?php if ($product_data['product_stock_status'] == 'instock') { ?>
								<span style="<?php if ($found_in_cart) {
													echo 'display:none;';
												} else {
													echo 'display:block;';
												} ?>"
									data-quantity="<?php if ($found_in_cart) {
								...
								<div style=" <?php if ($found_in_cart) {
													echo 'display:flex;';
												} else {
													echo 'display:none;';
												} ?>"
									...
									<input class="number_count number_<?php echo $product_data['product_id'] ?>" type="number" id=""
										value="<?php if ($found_in_cart) {
													echo $cart[$product_cart_id]['quantity'];
												} ?>" data-min="1" disabled />
									<button
										class="decrease <?php if ($found_in_cart) {
															if ($cart[$product_cart_id]['quantity'] == 1) {
																echo 'disabled';
															}
														} ?> decrease_<?php echo $product_data['product_id'] ?>"
										id=""
						...
						?>
							...
							<p data-quantity="<?php if ($found_in_cart) {
													echo $cart[$product_cart_id]['quantity'];
												} else {
													echo '0';
												} ?>"
								...
								<!-- <img src="<?php //echo $theme_settings['theme_url'];
												?>/assets/img/icons/add-cart.png" alt="" /> -->
								<span> <?php if ($language == 'en') {
											echo "View Product";
										} else {
											echo "اظهر المنتج";
										} ?></span>
