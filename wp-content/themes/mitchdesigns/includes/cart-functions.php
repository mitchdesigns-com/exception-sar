...
function mitch_apply_coupon(){
  ...
  if(!empty($coupon_id)){
    ...
    if(isset($errors) && !empty($errors)){
      ...
      $response = array('status' => 'success', 'cart_total' =>  number_format(WC()->cart->cart_contents_total ) . " SAR", 'cart_discount_div' => $cart_discount_div);
    }
  }else{
...
function mitch_remove_coupon(){
  ...
  $response        = array(
    'status'       => 'success',
    'cart_total'   =>  number_format(WC()->cart->cart_contents_total + $shipping_cost) . " SAR",
    'cart_count'   => WC()->cart->get_cart_contents_count(),
    'cart_content' => mitch_get_cart_content($lang),
  );
  echo json_encode($response);
      ...
      <div class="button_action">
          <a class="open_checkout" href="' ;
            ...
            <a href="' ;
              ...
              $cart_content = $cart_content . $checout_url . '"> '.cart_page('order_now' , $lang).' </a>
          </a>
