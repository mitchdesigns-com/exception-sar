...
<table class="shop_table woocommerce-checkout-review-order-table">
									...
									<div class="prices">
										...
										<div class="total-price" style="display:none;">
											<p><?php $Line_Price = (WC()->cart->get_product_subtotal($_product, $cart_item['quantity'])); ?>
												<?php $Line_Price = str_replace('SAR', 'ر.س',  $Line_Price); ?>
	...
	<tfoot>
		<tr class="cart-subtotal">
			<th><?php echo cart_page('price', $language); //esc_html_e( 'Subtotal', 'woocommerce' ); 
				?></th>
			<?php
			$cart_subtotal = str_replace('.00', '', WC()->cart->get_cart_subtotal());

			if ($language == 'ar') {
				$cart_subtotal = str_replace('SAR', 'ر.س',  $cart_subtotal);
			}
			?>
		...
		<?php foreach (WC()->cart->get_fees() as $fee) : ?>
			<tr class="fee">
				<?php if ($language != 'en') { ?>
					...
					<td><?php echo esc_html($fee->amount * -1) . 'ر.س' ?></td>
				<?php } else { ?>
					<th><?php echo esc_html(" Pay at Delivery"); ?></th>
					<td><?php echo esc_html($fee->amount * -1) . ' SAR' ?></td>
				<?php } ?>
							...
							<form class="checkout_coupon woocommerce-form-coupon" method="post" action="">
								...
								<div class="coupon">
									...
									<button type="submit" id="apply_coupon" class="button btn blue" name="apply_coupon"
										value="<?php esc_attr_e('Apply coupon', 'woocommerce'); ?>"><?php //echo($language == 'en')? 'Apply Now' : 'أدخلي الكود'
																										?></button>
								</div>
							</form>
							<p class="open-coupon">
								...
								<!-- <a href="#" class="btn blue"><?php //echo($language == 'en')? 'Apply Now' : 'أدخلي الكود'
																	?></a></p> -->
						</div>
					<?php } ?>
				</td>
		...
		<tr class="order-total">
			...
			if ($language == 'ar') {
				$cart_total = str_replace('SAR', "ر.س",  $cart_total);
			}
