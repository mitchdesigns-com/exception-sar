...
function add_custom_radio_options()
{
    ...
    if ($custom_cake_found) {
        ...
        <div class="new_downpayment_options">
            ...
            if ($language == 'en') {
                $full = " Full Payment " . WC()->cart->subtotal . 'SAR';
                $part = 'Pay Deposit at Delivery ' . $discount . ' SAR';
            } else {

                $full = "ادفع كامل المبلغ الان  " . WC()->cart->subtotal . 'ر.س';
                $part = 'ادفع عربون و الباقي عند الاستلام ' . $discount . 'ر.س';
            }
            ?>
...
function get_simple_product_skus()
{
    $args = array(
        'type' => 'simple',
        'limit' => -1,
